...
def vid_2_fast():
    bashVideoAlter4 = ("ffmpeg -i {0} -filter_complex [0:v]setpts=0.5*PTS[v];[0:a]atempo=2.0[a] -map [v] -map [a] {1}".format(vid2outputLoc,vid2AlteredLoc))
    process1 = subprocess.check_call(bashVideoAlter4.split(), stdout=subprocess.PIPE)

def vid_2_slow():
    bashVideoAlter5 = ("ffmpeg -i {0} -filter_complex [0:v]setpts=2.0*PTS[v];[0:a]atempo=0.5[a] -map [v] -map [a] {1}".format(vid2outputLoc,vid2AlteredLoc))
    process1 = subprocess.check_call(bashVideoAlter5.split(), stdout=subprocess.PIPE)

def vid_2_nothing():
    bashVideoAlter6 = ("cp {0} {1}".format(vid2outputLoc,vid2AlteredLoc))
    process1 = subprocess.check_call(bashVideoAlter6.split(), stdout=subprocess.PIPE)

def vid_3_fast():
    bashVideoAlter7 = ("ffmpeg -i {0} -filter_complex [0:v]setpts=0.5*PTS[v];[0:a]atempo=2.0[a] -map [v] -map [a] {1}".format(vid3outputLoc,vid3AlteredLoc))
    process1 = subprocess.check_call(bashVideoAlter7.split(), stdout=subprocess.PIPE)

def vid_3_slow():
    bashVideoAlter8 = ("ffmpeg -i {0} -filter_complex [0:v]setpts=2.0*PTS[v];[0:a]atempo=0.5[a] -map [v] -map [a] {1}".format(vid3outputLoc,vid3AlteredLoc))
